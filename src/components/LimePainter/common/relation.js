    ...
    watch: {
      css: {
        handler(v) {
          if (this.canvasId) {
            this.el.css =
              (typeof v == 'object' ? v : v && Object.assign(...styles(v))) ||
              {}
            this.canvasWidth =
              (this.el.css && this.el.css.width) || this.canvasWidth
            this.canvasHeight =
              (this.el.css && this.el.css.height) || this.canvasHeight
          }
        },
        immediate: true
      }
    ...
    methods: {
      bindRelation() {
        ...
        if (views.indexOf(this.el) !== -1) {
          this.parent.el.views = views.map(v =>
            v._uid == this._uid ? this.el : v
          )
        } else {
